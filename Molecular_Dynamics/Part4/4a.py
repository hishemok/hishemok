...
T = 300/119.7
#or 167.4/119.7 after finding new initial temp
    ...
    for i in range(n_):
        for j in range(n_):
            for k in range(n_):
                ...
                v[index,0,:] = np.random.normal(0,np.sqrt(T),size=(3))
                index += 1
...
def Temp(v):
    return 1/(3*N)*np.sum(v**2, axis=(0, 2))



import matplotlib.pyplot as plt

plt.plot(time,Temp(v))
plt.xlabel('t[s]')
plt.ylabel('T[k]')
plt.show()
